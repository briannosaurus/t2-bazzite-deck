#!/bin/bash
#
# Vendored from kansei-os/t2-atomic build_files/t2-enablement-ublue.sh
# (commit ef80157, 2026-08-10), Apache-2.0, see LICENSE.kansei-os.
# Modification: the "t2-bazzite-deck addition" block below.
...
# t2-bazzite-deck addition: Bazzite's own -ogc kernel build periodically gains new
# akmod-built kernel modules (evdi, gcadapter_oc, hid-fanatecff, kvmfr, nct6687d as of
# fc44 7.2.0-ogc4.1) that hard-pin to that exact kernel via the "kernel-uname-r" Requires
# and block the override below with a depsolve conflict. None are relevant on Apple
# hardware. Sweep dynamically by capability (not a hand-maintained name list) so a future
# Bazzite kernel rebuild adding yet another such module doesn't silently break this again.
mapfile -t _locked_kmods < <(rpm -q --qf '%{NAME}\n' --whatrequires kernel-uname-r 2>/dev/null | sort -u)
if [ "${#_locked_kmods[@]}" -gt 0 ]; then
    dnf5 -y remove "${_locked_kmods[@]}"
fi
